...
func main() {
	...
	var toPath string
	flag.StringVar(&toPath, "path", "dirpath", "tell me the path(like -path=/Users/ReadingFriends/Resources).")
	flag.Parse()

	if toPath == "dirpath" {
		logPrint("tell me the path(like -path=/Users/ReadingFriends/Resources).")
		return
	}
...
func CppyDir(fromPath, toPath string) error {
	...
	if err != nil {
		logPrint("create dir %v fail", toPath)
		return err
	}
	//生成目的目录
	var toFileMap = make(map[string]string) //
	err = filepath.Walk(toPath, func(path string, f os.FileInfo, err error) error {
		if f == nil {
			return err
		}
		if f.IsDir() {
		...
		temp := strings.Replace(path, toPath, "", 1)
		if strings.HasSuffix(temp, ".meta") {
			return nil
		}
		//logPrint("%v", temp) //
		toFileMap[temp] = path
	...
	if err != nil {
		fmt.Printf("CppyDir：filepath.Walk() returned %v\n", err)
		return err
	}
	...
	err = filepath.Walk(fromPath, func(path string, f os.FileInfo, err error) error {
		...
		temp := strings.Replace(path, fromPath, "", 1)
		bNeed := true
		if v, ok := toFileMap[temp]; ok {
			//检测MD5
			if GetFileMd5(v) == GetFileMd5(path) {
				bNeed = false
			}
		}
		if bNeed {
			//拷贝文件
			//创建文件夹
			tempdir := strings.Replace(toPath+temp, f.Name(), "", 1)
			err = os.MkdirAll(tempdir, os.ModePerm)
			if err != nil {
				logPrint("create dir %v fail", tempdir)
				return err
			}
			//文件是否在
			_, err = CopyFileByName(path, toPath+temp)
			if err != nil {
				logPrint("%v copy %v fail ", path, toPath+temp)
				return errors.New("error")
			}
			//拷贝
			//logPrint("%v", temp) //
		}
		delete(toFileMap, temp)
		return nil
	})
	if err != nil {
		fmt.Printf("CppyDir：filepath.Walk() returned %v\n", err)
	...
	//topath里清除剩下的
	for _, v := range toFileMap {
		//log.Println(k, v)
		err := os.Remove(v) //删除文件tevst.txt
		if err != nil {
			//如果删除失败则输出 file remove Error!
			fmt.Printf("CppyDir：Remove returned %v\n", err)
			return err
		}
	}
	//over清除空文件夹,咋清,

	var toDirMap = make(map[string]string) //
	err = filepath.Walk(toPath, func(path string, f os.FileInfo, err error) error {
		if f == nil {
			return err
		}
	...
	for _, k := range toDirMap {
		bEmpty := true
		err = filepath.Walk(k, func(path string, f os.FileInfo, err error) error {
			if f == nil {
				return err
			}
			if f.IsDir() {
		...
		if bEmpty {
			...
			err = os.RemoveAll(k)
			if err != nil {
				logPrint("RemoveAll dir %v fail", k)
				return err
			}
		}
	}
...
func GetFileMd5(path string) string {
	...
	return fmt.Sprint("", md5hash.Sum(nil))
}
